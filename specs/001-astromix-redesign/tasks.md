...
- [ ] T001 Refactor root layout to providers-only: strip nav/footer from `src/app/layout.tsx`, keep only html/body/fonts/PostHogProvider/CookieConsent
- [ ] T002 Create main route group layout at `src/app/(main)/layout.tsx` — move nav, footer, MobileNav, AuthNav here from old root layout
- [ ] T003 Move all existing pages into `src/app/(main)/` route group (page.tsx, dashboard/, moon/, zodiac/, compatibility/, horoscopes/, chart/, auth/, explore/) — preserve all URLs
- [ ] T004 Create quiz route group layout at `src/app/(quiz)/layout.tsx` — minimal layout with logo only, light theme CSS (quiz-theme class), no header/footer/mobile nav
- [ ] T005 Add quiz light theme CSS variables and `body:has(.quiz-theme)` overrides to `src/app/globals.css`
- [ ] T006 Verify build passes after route group refactor: `npm run build`
...
- [ ] T011 [P] Create product types and static data at `src/lib/products/types.ts` — define Product, ProductCategory interfaces and PRODUCT_CATEGORIES constant
- [ ] T012 [P] Create product catalog static data at `src/lib/products/data.ts` — all 16 products with slug, name_uk, category, description_uk, value_props_uk, price_usd, icon, sort_order
- [ ] T013 [P] Create quiz types at `src/lib/quiz/types.ts` — QuizState, QuizAction, QuizStep interfaces, TOTAL_STEPS=6 constant
- [ ] T014 [P] Create quiz reducer at `src/lib/quiz/reducer.ts` — quizReducer with NEXT/BACK/SET_ANSWER/RESET actions, sessionStorage persistence on each dispatch
- [ ] T015 [P] Create quiz validation at `src/lib/quiz/validation.ts` — per-step validators (step 0: birthDate required, step 2: city+lat required, step 3: time or unknown checked, step 4: name+gender, step 5: email+consent)
- [ ] T016 [P] Create quiz tracking helpers at `src/lib/quiz/tracking.ts` — getOrCreateQuizSessionId(), trackQuizStart(), trackQuizStep(), trackQuizAbandonment() using existing PostHog `track()` function
- [ ] T017 [P] Add new analytics events to `src/lib/analytics/events.ts` — quiz events (QUIZ_STARTED, QUIZ_STEP_COMPLETED, QUIZ_ABANDONED, QUIZ_COMPLETED), product events (PRODUCT_PAGE_VIEWED, PRODUCT_CARD_CLICKED), paywall events (PAYWALL_VIEWED, PAYWALL_CTA_CLICKED), email events (EMAIL_SUBSCRIPTION_SUBMITTED)
- [ ] T018 Verify build passes after foundational work: `npm run build`
...
- [ ] T019 [P] [US1] Create QuizProgressBar component at `src/components/quiz/QuizProgressBar.tsx` — 6 dots/steps, active state with zorya-violet, light theme styling
- [ ] T020 [P] [US1] Create QuizStep1Birthday component at `src/components/quiz/QuizStep1Birthday.tsx` — day/month/year select dropdowns (Ukrainian month names), "Далі" button, zodiac calculation on advance
- [ ] T021 [P] [US1] Create QuizStep2Sign component at `src/components/quiz/QuizStep2Sign.tsx` — display calculated zodiac sign with ZodiacIcon + Ukrainian name, brief fun fact, "Далі" and "Назад" buttons
- [ ] T022 [P] [US1] Create QuizStep3Birthplace component at `src/components/quiz/QuizStep3Birthplace.tsx` — reuse existing CitySearch component for Nominatim autocomplete, store lat/lng/city/countryCode
- [ ] T023 [P] [US1] Create QuizStep4BirthTime component at `src/components/quiz/QuizStep4BirthTime.tsx` — time input (24h format for UA), "Я не знаю точний час народження" checkbox, reassuring helper text
- [ ] T024 [P] [US1] Create QuizStep5NameGender component at `src/components/quiz/QuizStep5NameGender.tsx` — name text input, gender radio (Жіноча/Чоловіча), Yin/Yang explanation text
- [ ] T025 [P] [US1] Create QuizStep6Email component at `src/components/quiz/QuizStep6Email.tsx` — email input, user agreement + privacy policy checkbox, "Відкрити гороскоп" submit button
- [ ] T026 [US1] Create quiz page at `src/app/(quiz)/quiz/page.tsx` — compose all 6 step components with useReducer (from T014), AnimatePresence slide transitions, progress bar, keyboard navigation (Enter to advance), beforeunload abandonment tracking, resume from sessionStorage on return (FR-016: abandoned quizzes resume from last step)
- [ ] T027 [US1] Create POST `/api/quiz/session` route at `src/app/api/quiz/session/route.ts` — validate input, save quiz session to Supabase quiz_sessions table, return session ID and zodiac info (per contracts/quiz-api.md)
- [ ] T028 [US1] Create POST `/api/quiz/complete` route at `src/app/api/quiz/complete/route.ts` — fetch quiz session from Supabase, call Astrology API SDK getNatalChart() for natal data, call OpenAI for 2-3 aspect interpretations in Ukrainian, save mini_horoscope_data to quiz_sessions, return full mini-horoscope response (per contracts/quiz-api.md)
- [ ] T029 [US1] Create MiniHoroscope component at `src/components/quiz/MiniHoroscope.tsx` — display natal chart SVG (reuse NatalChartWheel), show 2-3 key aspects with planet names (Ukrainian), degree positions, and AI-generated interpretation text
- [ ] T030 [US1] Create PaywallSection component at `src/components/product/PaywallSection.tsx` — display product tiers from products data, prices, benefit list, "coming soon" message on click (payment deferred), track PAYWALL_VIEWED and PAYWALL_CTA_CLICKED events
- [ ] T031 [US1] Integrate mini-horoscope + paywall into quiz completion flow in `src/app/(quiz)/quiz/page.tsx` — after step 6 submit, call /api/quiz/session then /api/quiz/complete, show loading state, then render MiniHoroscope + PaywallSection below
- [ ] T032 [US1] Verify quiz flow end-to-end: walk through all 6 steps, check PostHog events fire, verify mini-horoscope renders with real data, verify paywall displays
...
- [ ] T036 [P] [US2] Create ProductCatalog component at `src/components/landing/ProductCatalog.tsx` — category filter pills (Популярне/Призначення/Кохання/Діти/Здоров'я/Майбутнє/Гроші), product cards in CSS scroll-snap carousel, AnimatePresence on filter change, track product_card_clicked
...
- [ ] T041 [P] [US2] Create StatsSection component at `src/components/landing/StatsSection.tsx` — animated counters (100,000+ гороскопів, 30+ років, 97% відгуків), CTA
...
- [ ] T043 [P] [US2] Create BlogPreviewSection component at `src/components/landing/BlogPreviewSection.tsx` — 3 placeholder article cards (date, tag, title), "Більше статей" link to /blog
- [ ] T044 [P] [US2] Create EmailSubscriptionSection component at `src/components/landing/EmailSubscriptionSection.tsx` — email + name inputs, submit button, -25% discount messaging, benefits list, calls POST /api/email/subscribe on submit
...
- [ ] T047 [US2] Rewrite landing page at `src/app/(main)/page.tsx` — compose all 11 section components in order (Hero → PainPoints → HowItWorks → ProductCatalog → BookOfLife → Testimonials → Stats → AccountBenefits → BlogPreview → EmailSubscription → SeoText), add SEO metadata
...
- [ ] T050 [P] [US3] Create ProductForm component at `src/components/product/ProductForm.tsx` — birth data form (name, gender, DOB, time, city) pre-filled from quiz session via sessionStorage, "Продовжити" button
...
- [ ] T056 [P] [US4] Create DailySummary component at `src/components/dashboard/DailySummary.tsx` — personalized greeting, today's date, love/career/finance percentage bars (fetched from /api/daily-horoscope for user's zodiac sign via Astrology API), "Фокус дня" indicator
...
- [ ] T066 [US5] Verify navigation: desktop dropdowns open on hover with all products/tools listed, mobile hamburger works, footer renders 4 columns, all links resolve
...
- [ ] T069 [US6] Create Daily Horoscope page at `src/app/(main)/daily/page.tsx` — hero, zodiac sign selector (12 ZodiacIcon buttons), display daily forecast from /api/daily-horoscope with categories, CTA to /horoscope/monthly, SEO metadata
...
## Phase 9: User Story 7 — Blog, Reviews & Content (Priority: P3)

**Goal**: Blog listing, reviews page with ratings, and 2026 yearly horoscope overview

**Independent Test**: Visit /blog → see article cards. Visit /reviews → see testimonials with star ratings. Visit /2026 → see yearly overview.

### Implementation for User Story 7

- [ ] T074 [P] [US7] Create BlogCard component at `src/components/blog/BlogCard.tsx` — GlassCard with date, category tag pill, title, excerpt, "Читати" link
- [ ] T075 [P] [US7] Create BlogCategoryFilter component at `src/components/blog/BlogCategoryFilter.tsx` — filter pills (Астрологія, Зодіак, Гороскопи, Зоря), "Усі" default
- [ ] T076 [US7] Create blog listing page at `src/app/(main)/blog/page.tsx` — BlogCategoryFilter + grid of BlogCards, placeholder articles (3-6 static entries), CTA to quiz in sidebar, SEO metadata
- [ ] T077 [P] [US7] Create GET/POST `/api/reviews` route at `src/app/api/reviews/route.ts` — GET: query published reviews from Supabase, optional product filter, return with average_rating and total count. POST: authenticated insert (per contracts/products-api.md)
- [ ] T078 [US7] Create reviews page at `src/app/(main)/reviews/page.tsx` — overall stats header (total reviews, avg rating), product filter dropdown, review cards (author, 5-star rating, text, date, product), "Залишити відгук" form for authenticated users, seed 5-10 sample reviews, SEO metadata
- [ ] T079 [US7] Create 2026 yearly horoscope page at `src/app/(main)/2026/page.tsx` — hero "Ваш Повний Путівник по 2026 році", major planetary transits section, quick forecast per zodiac sign (12 GlassCards with ZodiacIcon), CTA to /horoscope/2026, SEO metadata
- [ ] T080 [US7] Verify content pages: /blog shows articles with filter, /reviews shows ratings and filter, /2026 shows all 12 signs

**Checkpoint**: Content pages live — blog, reviews, and yearly overview drive SEO and trust
