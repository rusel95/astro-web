...
import { LucideIcon, User, Briefcase, Heart, Activity, TrendingUp, Sparkles, Baby, ChevronRight, CheckCircle2, Lock } from 'lucide-react';
          ...
          <motion.button
            ...
            className={`w-full flex items-center gap-4 p-4 rounded-2xl border transition-all group backdrop-blur-sm
              ${isSelected
                ...
                : 'bg-white/[0.03] border-white/[0.07] hover:border-white/15 hover:bg-white/5'
              }
            `}
            style={{ opacity: isGenerated || isSelected ? 1 : 0.65 }}
          >
            ...
            <div className="flex-1 text-left">
              ...
              <div className="text-xs text-text-muted mt-0.5">
                {isGenerated ? area.description : 'Натисніть щоб згенерувати'}
              </div>
            </div>
            <div className="shrink-0">
              {isGenerated ? (
                ...
                <ChevronRight size={18} strokeWidth={1.75} className="text-zorya-violet" />
              ) : (
                <Lock size={15} strokeWidth={1.75} className="text-text-muted group-hover:text-zorya-violet transition-colors" />
              )}
            </div>
          </motion.button>
        );
