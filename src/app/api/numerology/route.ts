...
export async function POST(req: NextRequest) {
  try {
    ...
    const [coreNumbers, comprehensiveReport] = await Promise.all([
      client.numerology.getCoreNumbers({ subject: numerologySubject, options, language: 'uk' } as any).catch((e: unknown) => {
        Sentry.captureException(e, { tags: { route: 'numerology', call: 'getCoreNumbers' }, level: 'warning' });
        return null;
      }),
      client.numerology.getComprehensiveReport({ subject: numerologySubject, options, language: 'uk' } as any).catch((e: unknown) => {
