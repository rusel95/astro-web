...
export async function POST(req: NextRequest) {
  try {
    ...
    const [transitChart, transitChartSvg, transitReport, natalTransitReport, natalTransits] = await Promise.all([
      ...
      client.svg.getTransitChartSvg({ subject, transit_date: transitDate, options, language: 'uk' } as any, { responseType: 'text' } as any).catch((e: unknown) => {
        Sentry.captureException(e, { tags: { route: 'transit', call: 'getTransitChartSvg' }, level: 'warning' });
        return null;
      }),
      client.analysis.getTransitReport({ subject, transit_date: transitDate, options, language: 'uk' } as any).catch((e: unknown) => {
        Sentry.captureException(e, { tags: { route: 'transit', call: 'getTransitReport' }, level: 'warning' });
        return null;
      }),
      client.analysis.getNatalTransitReport({ subject, transit_date: transitDate, options, language: 'uk' } as any).catch((e: unknown) => {
        Sentry.captureException(e, { tags: { route: 'transit', call: 'getNatalTransitReport' }, level: 'warning' });
        return null;
      }),
      client.charts.getNatalTransits({ subject, start_date: transitDate, end_date: addDays(transitDate, 30), options, language: 'uk' } as any).catch((e: unknown) => {
