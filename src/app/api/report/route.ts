...
const AREA_NAMES: Record<ReportArea, string> = {
  ...
  children: 'Дитячий гороскоп',
  marriage: 'Коли одружуся — прогностичний аналіз',
};
...
const AREA_FOCUS: Record<ReportArea, string> = {
  ...
  children: "характер та темперамент дитини, природні таланти та здібності, стиль навчання, емоційні потреби, рекомендації для батьків щодо виховання та підтримки розвитку дитини",
  marriage: "ПРОГНОСТИЧНИЙ аналіз шлюбу: сприятливі періоди для шлюбу на основі транзитів Юпітера до 7 дому та Венери, прогресованої Венери та Місяця, дирекцій до Десценденту. Вказати конкретні роки та періоди коли шлюб найбільш ймовірний. Аналіз 7 дому (партнерство), Венери (кохання), Юнони (шлюб), Вертексу. Які транзити зовнішніх планет (Юпітер, Сатурн, Уран) по 7 дому і до натальної Венери створюють вікно для шлюбу. НЕ описувати характер людини, а дати ПРОГНОЗ ЧАСУ для одруження",
};
